	...
	if user is None:
		...
		if target == machine:
			...
			if balance < amount:
				...
				bot.send_message(chat_id=update.message.chat_id, text="@{0} tipped @{1} of {2} PYRK".format(user, target, amount))
		else: 
			bot.send_message(chat_id=update.message.chat_id, text="Error that user is not applicable.")

def balance(bot,update):
	quote_page = requests.get('https://www.worldcoinindex.com/coin/pyrk')
	...
	if user is None:
		...
		bot.send_message(chat_id=update.message.chat_id, text="@{0} your current balance is: {1} PYRK ≈  ${2}".format(user,balance,fiat_balance))
...
def price(bot,update):
	...
	#btc = soup[80:]
	#sats = btc[:-2]
	#bot.send_message(chat_id=update.message.chat_id, text="Pyrk is valued at {0} Δ {1} ≈ {2}".format(price,percent,sats) + " ฿")
	bot.send_message(chat_id=update.message.chat_id, text="Pyrk is valued at {0} Δ {1}".format(price,percent) )
...
def marketcap(bot,update):
	...
	name = name_box.text.replace("\n","")
	mc = re.sub(r'\n\s*\n', r'\n\n', name.strip(), flags=re.M)
	bot.send_message(chat_id=update.message.chat_id, text="The current market cap of Pyrk is valued at {0}".format(mc))
